...
##                                    #}end of with open
                                    ...
                                    with zipfile.ZipFile(path): #{ opening the zip file using 'zipfile.ZipFile' class
                                        with open('dx.txt','a+') as fob: #{ save the file that is being downloaded
                                            ...
                                            fob.close()
                                        #} end of with open
                                        print(thrd,": File is Ok")  
                                    #}end of with zipfile
                                #}end of if
                                else:#{
                                    print(thrd,':',exts,'Wrong File Type!')
            ...
            #}end of TorClient                                    
            retry=False
            print(thrd,': Download Complete!')
            
        #}end of try
        except:#{
            retry = True
            max_retry = max_retry - 1
            print(thrd,': Retrying... (%i)'%max_retry)
        #}end of except            
    #}end of while
...
def url2data(thrd):#{
    ...
    while len(r):#{
        ...
        for i in tqdm(t,desc='Loads'): i.start()

        for i in tqdm(t,desc='Ended'): i.join()
